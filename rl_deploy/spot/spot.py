...
class Spot:
    ...
    def _command_stream_loop(
        ...
        """coroutine needed for command stream. repeatedly calls timing_policy
        to block until next dt and then yields the result of command_policy once.
        Activates joint control after the first command is sent.
    ...
    def _activate_joint_control(self):
        """Activate full body joint control via a standard RobotCommand.
        Called inline from the command stream after the first command is sent.
        """
        ...
        command_client.robot_command(joint_command)
